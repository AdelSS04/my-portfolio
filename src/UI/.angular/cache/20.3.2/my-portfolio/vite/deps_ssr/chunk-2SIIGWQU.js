...
import {
  ...
  Router,
  loadChildren
} from "./chunk-2DSQYLCN.js";
import {
  BrowserDomAdapter,
  BrowserModule,
  EVENT_MANAGER_PLUGINS,
  EventManagerPlugin
} from "./chunk-VEGX3WPT.js";
import {
...
function requireSelect() {
  ...
  (function(module, exports) {
    ...
    var truncateUrl = function(url, num) {
      return url.replace(/^(?:\w+:\/\/|\/+)/, "").replace(/(?:\/+|\ (function() {
        return function() {
          return true;
        };
      })(),
...
function parseSelector(subselects2, selector, selectorIndex) {
  ...
  loop: while (selectorIndex < selector.length) {
    ...
    switch (firstChar) {
      ...
      default: {
        if (selector.startsWith("", selectorIndex + 2);
          if (endIndex < 0) {
...
var MEDIA_TYPES =  new Set(["all", "print", "screen", "speech"]);
var MEDIA_KEYWORDS =  new Set(["and", "not", ","]);
...
var selectorTokensCache =  new Map();
...
var IGNORED_LOGS =  new Set(["Angular is running in development mode."]);
...
var VALID_REDIRECT_RESPONSE_CODES =  new Set([301, 302, 303, 307, 308]);
...
function buildServerConfigRouteTree({ routes, appShellRoute }) {
  ...
  for (const _a2 of serverRoutes) {
    ...
    if ("getPrerenderParams" in metadata && (path2.includes(" new Set();
