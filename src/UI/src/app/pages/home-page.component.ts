    ...
    <main id="main">
      <section id="home" class="hero">
        ...
        <div class="hero-content">
          ...
          <div class="hero-actions"><a class="button button-primary" href="#work">View projects <lucide-icon [img]="ArrowUpRight" /></a><a class="button button-ghost" href="#contact">Start a conversation <lucide-icon [img]="ArrowUpRight" /></a></div>
        </div>
          ...
          <div #projectTrack class="project-track" (scroll)="syncProjectIndex(projectTrack)">
            @for (project of projects; track project.title; let index = $index) {
              <article class="project-card" [class.active]="selectedProject() === index" [class.project-card--featured]="project.featured" tabindex="0" [attr.aria-expanded]="selectedProject() === index" (mouseenter)="selectedProject.set(index)" (focusin)="selectedProject.set(index)" (click)="selectProject(index)">
                <div class="project-media"><img [src]="projectImage(project.image, index)" [alt]="project.title + ' project preview'" width="1280" height="720" loading="lazy" decoding="async"><div class="project-shade"></div><span>{{ project.featured ? 'Featured · ' + project.category : project.category }}</span></div>
                <div class="project-body"><div class="project-title"><h3>{{ project.title }}</h3><small>{{ project.year }}</small></div><p>{{ project.description }}</p><strong>{{ project.impact }}</strong><div class="tags">@for (tag of project.technologies; track tag) { <span>{{ tag }}</span> }</div><div class="project-links">@if (project.liveUrl) { <a [href]="project.liveUrl" target="_blank" rel="noopener noreferrer"><span>Live project</span><lucide-icon [img]="ArrowUpRight" /></a> }@if (project.githubUrl) { <a [href]="project.githubUrl" target="_blank" rel="noopener noreferrer"><span>Source</span><lucide-icon [img]="Github" /></a> }</div></div>
              </article>
            }
          </div>
      ...
      <section id="services" class="section services reveal-section">
        ...
        <div class="capability-shell"><div class="capability-tabs" role="tablist" aria-label="Capabilities">@for (service of services; track service.title; let index = $index) { <button type="button" role="tab" [attr.aria-selected]="selectedService() === index" [class.active]="selectedService() === index" (click)="selectedService.set(index)">{{ service.title }}</button> }</div>@for (service of [currentService()]; track service.title) { <div class="capability-panel" role="tabpanel"><div class="capability-copy"><span>{{ service.eyebrow }}</span><h3>{{ service.title }}</h3><p>{{ service.copy }}</p><div class="capability-tools">{{ service.tools }}</div><a href="#contact">Talk about this work <lucide-icon [img]="ArrowUpRight" /></a></div><div class="capability-proof"><ul>@for (benefit of service.benefits; track benefit) { <li>{{ benefit }}</li> }</ul><p><small>PROOF FROM MY WORK</small>{{ service.proof }}</p></div><div class="capability-mark" aria-hidden="true">{{ service.title.charAt(0) }}</div></div> }</div>
      </section>
      ...
      <section id="experience" class="section experience reveal-section">
        ...
        <aside class="experience-lab"><div><span>Outside client work</span><h3>Homelab as a platform practice.</h3><p>I run a two-node Proxmox environment to keep learning where production work gets real: provisioning, deployments, monitoring, logs, access, and recovery.</p></div><ul><li><b>Provision</b><span>Pulumi + Ansible</span></li><li><b>Deploy</b><span>Docker Compose + Komodo</span></li><li><b>Observe</b><span>Grafana + Loki</span></li></ul><a href="#work" (click)="selectProject(2)">See the homelab <lucide-icon [img]="ArrowUpRight" /></a></aside>
      </section>
      ...
      <section id="writing" class="section writing reveal-section"><div class="section-heading"><div><div class="section-kicker">Writing</div><h2>Notes from<br><em>real software.</em></h2></div><a class="text-link" href="https://blog.adellajil.com" target="_blank" rel="noopener noreferrer">All writing <lucide-icon [img]="ArrowUpRight" /></a></div><div class="writing-grid"><a class="featured-article" href="https://blog.adellajil.com/blog/why-i-started-treating-my-homelab-like-a-small-platform/" target="_blank" rel="noopener noreferrer"><span>Featured · Cloud / DevOps</span><h3>Why I Started Treating My Homelab Like a Small Platform</h3><p>What changed when I stopped treating home infrastructure as a set of containers and started giving each part a clear job.</p><small>9 min read <b>↗</b></small></a><div class="article-list"><a href="https://blog.adellajil.com/blog/privacy-safe-two-node-homelab-tour/" target="_blank" rel="noopener noreferrer"><span>Cloud / DevOps · 10 min</span><h3>A Privacy-Safe Tour of My Two-Node Homelab</h3><b>↗</b></a><a href="https://blog.adellajil.com/blog/exposing-securing-loki-log-ingestion/" target="_blank" rel="noopener noreferrer"><span>Observability · 18 min</span><h3>Exposing and Securing Loki Log Ingestion</h3><b>↗</b></a><a href="https://blog.adellajil.com/blog/azure-frontdoor-cache-warmup/" target="_blank" rel="noopener noreferrer"><span>Cloud · 13 min</span><h3>From Cache Miss to Cache Hit with Azure Front Door</h3><b>↗</b></a></div></div></section>

      <section id="testimonials" class="testimonials reveal-section"><div class="section testimonial-layout"><div><div class="section-kicker">Recommendations</div><h2>What colleagues<br><em>say about the work.</em></h2><p>Direct feedback from people who have worked with me on software teams.</p><div class="recommendation-note"><span>{{ recommendations.length.toString().padStart(2, '0') }}</span><small>Professional recommendations</small></div></div><div class="quote-stack">@for (recommendation of recommendations; track recommendation.name; let index = $index) { <blockquote [class.quote-featured]="index === 0"><span class="quote-mark" aria-hidden="true">“</span><p>{{ recommendation.quote }}</p><footer><strong>{{ recommendation.name }}</strong><span>{{ recommendation.role }}</span></footer></blockquote> }</div></div></section>

      <section id="freelance" class="section freelance reveal-section">
        <div class="section-heading"><div><div class="section-kicker">Freelance</div><h2>Un site web, un outil,<br><em>un problème réglé.</em></h2></div><p>Je crée des sites web et des outils sur mesure pour les petites entreprises du Québec. En français, sans jargon, avec un seul interlocuteur.</p></div>
        <div class="freelance-grid"><article class="freelance-card"><h3>Sites web professionnels</h3><p>Création ou refonte de votre site vitrine : rapide, moderne et bien référencé, pensé pour transformer les visiteurs en clients.</p><div class="tags"><span>Design mobile</span><span>SEO</span><span>Formulaires de contact</span></div></article><article class="freelance-card"><h3>Outils internes sur mesure</h3><p>Tableaux de bord, automatisations et gestion : des outils pensés pour votre équipe, qui font gagner du temps chaque jour.</p><div class="tags"><span>Tableaux de bord</span><span>Automatisation</span><span>Accès par rôle</span></div></article><article class="freelance-card"><h3>Applications web</h3><p>Applications robustes en .NET et Angular : espaces clients, réservation en ligne, commandes ou gestion d'inventaire.</p><div class="tags"><span>API</span><span>Espaces clients</span><span>Paiements Stripe</span></div></article><article class="freelance-card"><h3>Cloud & infrastructure</h3><p>Hébergement, sécurité et sauvegardes sur Azure : une infrastructure fiable qui tourne sans que vous ayez à vous en occuper.</p><div class="tags"><span>Hébergement</span><span>Sauvegardes</span><span>Sécurité</span></div></article></div>
        <div class="process-path freelance-steps"><article><span>Appel découverte</span><h3>On discute de votre besoin.</h3><p>Sans engagement : on cerne le problème et ce qu'un bon résultat changerait pour vous.</p></article><article><span>Proposition claire</span><h3>Vous savez ce que vous payez.</h3><p>Périmètre, délais et prix fixés par écrit avant de commencer. Pas de surprise.</p></article><article><span>Réalisation</span><h3>On construit par étapes visibles.</h3><p>Vous voyez l'avancement en continu et on ajuste ensemble au besoin.</p></article><article><span>Suivi</span><h3>Je reste disponible après.</h3><p>Mise en ligne, formation rapide et support pour les premiers mois.</p></article></div>
        <div class="freelance-cta"><a class="button button-primary" href="#contact">Discuter de mon projet <lucide-icon [img]="ArrowUpRight" /></a><p>Chaque projet est différent — parlons-en, sans engagement.</p></div>
      </section>
...
export class HomePageComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  readonly moreNav = [{ id: 'credentials', label: 'Credentials' }, { id: 'process', label: 'How I work' }, { id: 'open-source', label: 'Open source' }, { id: 'testimonials', label: 'Recommendations' }, { id: 'faq', label: 'FAQ' }, { id: 'freelance', label: 'Freelance' }];
